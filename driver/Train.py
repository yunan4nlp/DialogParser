...
def train(train_instances, dev_instances, test_instances, parser, vocab, config):
    global_step = 0
    best_dev_las = 0
    ...
    for iter in range(config.train_iters):
        ...
        for onebatch in data_iter(train_instances, batch_size=config.train_batch_size, shuffle=True):
            parser.train()
            word_indexs, extword_indexs, word_lengths, edu_lengths, arc_masks = batch_data_variable(onebatch, vocab)
            feats = batch_feat_variable(onebatch, vocab)

            gold_arcs, gold_rels = batch_label_variable(onebatch, vocab)
            parser.forward(word_indexs, extword_indexs, word_lengths, edu_lengths, arc_masks, feats)
            ...
            if batch_iter % config.validate_every == 0 or batch_iter == batch_num:
                predict(dev_instances, parser, vocab, config, config.dev_file + "." + str(global_step))
                print("Dev:")
                dev_las = evaluation(config.dev_file, config.dev_file + "." + str(global_step))

                predict(test_instances, parser, vocab, config, config.test_file + "." + str(global_step))
                print("Test:")
...
def predict(instances, parser, vocab, config, outputFile):
    start = time.time()
    parser.eval()
    pred_instances = []
    for onebatch in data_iter(instances, batch_size=config.test_batch_size, shuffle=False):
        ...
        pred_arcs, pred_rels = parser.parse(word_indexs, extword_indexs, word_lengths, edu_lengths, arc_masks,
                                            dialog_feats)
...
if __name__ == '__main__':
    ...
    global_encoder = GlobalEncoder(vocab, config)
    global_encoder.word_encoder.initial_pretrain(vec)
    ...
    parser = DialogDP(global_encoder, state_encoder, structured_encoder, decoder, config)

    train(train_instances, dev_instances, test_instances, parser, vocab, config)
