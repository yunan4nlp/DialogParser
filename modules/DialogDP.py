...
class DialogDP(object):
    ...
    '''
    def forward(self, word_indexs, extword_indexs, word_lengths, edu_lengths, arc_masks, feats):
        if self.use_cuda:
            word_indexs = word_indexs.cuda()
            arc_masks = arc_masks.cuda()
            extword_indexs = extword_indexs.cuda()
        ...
        edu_represents, edu_outputs = self.global_encoder(word_indexs, extword_indexs, word_lengths, edu_lengths)
        _, _, arc_logits, rel_logits = self.decode(edu_represents, edu_outputs, edu_lengths, arc_masks, feats)
    ...
    def parse(self, word_indexs, extword_indexs, word_lengths, edu_lengths, arc_masks, feats):
        if self.use_cuda:
            word_indexs = word_indexs.cuda()
            extword_indexs = extword_indexs.cuda()
            arc_masks = arc_masks.cuda()
            feats = feats.cuda()
        edu_represents, edu_outputs = self.global_encoder(word_indexs, extword_indexs, word_lengths, edu_lengths)
        pred_arcs, pred_rels, _, _ = self.decode(edu_represents, edu_outputs, edu_lengths, arc_masks, feats)
        return pred_arcs, pred_rels


    def decode(self, edu_represents, edu_outputs, edu_lengths, arc_masks, feats):
        cur_step = 0
        arc_logits = []
        rel_logits = []
        pred_arcs = None
        ...
        while not self.is_finished(cur_step, edu_lengths):
            ...
            state_hidden = self.state_encoder(cur_step, s_hidden, edu_represents, edu_outputs, edu_lengths, cur_feats)
            arc_logit, rel_logit = self.decoder(state_hidden, cur_arc_masks)
            arc_logits.append(arc_logit.unsqueeze(1))
            rel_logits.append(rel_logit.unsqueeze(1))
